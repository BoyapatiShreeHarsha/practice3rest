...
describe("Test for Page2", () => {
    test("test for back button", () => {
        const setData = jest.fn();

        render(<BrowserRouter><Page2 data={data1} setData={setData} /></BrowserRouter>)

        const backBtnEle = screen.getByTestId("backBtn");
        fireEvent.click(backBtnEle);
        expect(mockedUsedNavigate).toHaveBeenCalledWith("/");
    })

    test("working of border btn", async () => {
        const setData = jest.fn((data) => data);

        mockAxios.onGet("/alpha?codes=dji").reply(200, data2);

        const { rerender } = render(<BrowserRouter><Page2 data={data1} setData={setData} /></BrowserRouter>)

        const borderBtnEle = screen.getByTestId("borderbtn0");
        fireEvent.click(borderBtnEle);
        rerender(<BrowserRouter><Page2 data={data2} setData={setData} /></BrowserRouter>)
        await act(async () => {
            await waitFor(() => expect(screen.getByText(/Cyprus/i)).toBeInTheDocument());
        });
    })
});